...
def scan(sentence): # create function called "scan" and parameter "sentence"
        ...
        for word in words: # iterate a for loop with the variable "word" to process each index in the "words" variable
            ...
            except ValueError: # if no indices in the variable "number" can be converted to an integer, then
                if word in lexicon: # if the variable "word" is a valid index in the "lexicon" dictionary, 
                    token = lexicon[word] # then match the valid index to the "lexicon" dictionary and assign it to the variable "token"    
